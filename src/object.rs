...
impl Object {
  ...
  pub fn with_prototype_and_properties<'s>(
    ...
    let names = crate::handle::as_slice_of_raw_ptrs(names);
    let values = crate::handle::as_slice_of_raw_ptrs(values);
    unsafe {
      scope.cast_local(|sd| {
...
impl Array {
  ...
  pub fn new_with_elements<'s>(
    ...
    let elements = crate::handle::as_slice_of_raw_ptrs(elements);
    unsafe {
      scope.cast_local(|sd| {
