...
- ``v8_enable_multi_cage``: Builds pointer compression in multi-cage mode, giving each isolate group its own pointer compression cage. This is what ``v8::IsolateGroup::create()`` requires; without it ``v8::IsolateGroup::can_create_new_groups()`` returns false and only the default group exists. Use it to get past the 4GB per-group heap ceiling that pointer compression otherwise imposes, by spreading isolates over several groups. This mode turns off partition_alloc and the external code space, and is mutually exclusive with ``v8_enable_sandbox``, which requires a shared cage. Enabling it also implies pointer compression. Unlike the prebuilt ``_ptrcomp`` archives, nothing is published for this configuration, so ``V8_FROM_SOURCE=1`` is required and the build script will refuse to proceed without it rather than silently linking a shared-cage library.
