...
fn set_host_initialize_import_meta_object_callback() {
...

    {
        ...
        let result = module.instantiate_module(context, unexpected_module_resolve_callback);
        assert!(result.is_some());
...
fn script_origin() {
...

    {
        ...
        let mut script = v8::Script::compile(scope, context, source, Some(&script_origin)).unwrap();
        source.to_rust_string_lossy(scope);
...
fn object_template_from_function_template() {
    ...
    let mut isolate = v8::Isolate::new(params);
    {
        let mut hs = v8::HandleScope::new(&mut isolate);
        let scope = hs.enter();
        let mut function_templ = v8::FunctionTemplate::new(scope, fortytwo_callback);
...
fn function() {
...

    {
        ...
        let function =
            v8::Function::new(scope, context, fn_callback2).expect("Unable to create function");
...
fn compile_specifier_as_module_resolve_callback<'a>(
    ...
    let module = v8::script_compiler::compile_module(scope.isolate(), source).unwrap();
...
fn module_evaluation() {
    ...
    let mut isolate = v8::Isolate::new(params);
    {
       ...
       import 'Object.expando *= 2';",
        ...
        let result =
            module.instantiate_module(context, compile_specifier_as_module_resolve_callback);
...
fn external_references() {
    ...
    // the value 3.
    let startup_data = {
        let mut snapshot_creator = v8::SnapshotCreator::new(Some(&EXTERNAL_REFERENCES));
        {
            // TODO(ry) this shouldn't be necessary. workaround unfinished business in
            // the scope type system.
            let mut isolate = unsafe { snapshot_creator.get_owned_isolate() };
    ...
    // value.
    {
        ...
        let mut isolate = v8::Isolate::new(params);
        {
            ...
            let result = eval(scope, context, "if(F() != 'wrong answer') throw 'boom1'");
            assert!(result.is_none());

            let result = eval(scope, context, "if(F() != 'Hello callback!') throw 'boom2'");
            assert!(result.is_some());
        }
        // TODO(ry) WARNING! startup_data needs to be kept alive as long the isolate
        // using it. See note in CreateParams::set_snapshot_blob
...
fn value_checker() {
    ...
    let mut isolate = v8::Isolate::new(params);
    {
        ...
        let value = eval(scope, context, "(function(){return arguments})()").unwrap();
...
fn try_from_local() {
    ...
    let mut isolate = v8::Isolate::new(params);
    {
        ...
        {
            let value: v8::Local<v8::Value> = eval(scope, context, "(() => {})").unwrap();
